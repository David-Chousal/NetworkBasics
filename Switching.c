...
* Title: Lab2 – Switching.c
* Description: This program computes the switching senario
*/
...
int main(int argc, char *argv[]) {
   ...
   pPSusers = nPSusers / nCSusers;
   pPSusersNotBusy = 1 - pPSusers;
   double pAllNotBusy = pow(pPSusersNotBusy, nPSusers);
   double pOneBusy = nPSusers * pPSusers * pPSusersNotBusy;
   double pExactlyOne = pOneBusy * pPSusersNotBusy;
   double pTenBusy = pow(pPSusers, 10) * pow(pPSusersNotBusy, nPSusers - 10);
   double pAnyTenBusy = 1 - pow(pPSusersNotBusy, nPSusers - 10);
   double pTenMoreBusy = 1 - pow(pPSusersNotBusy, nPSusers - 10);
